"""
H3 Long Videos REF2VA  (reference images + one prompt -> long video+audio)
==========================================================================
The FL2VA sampler with H3's ref2va task added: connect up to four reference
images and the shots are conditioned on them, so a character keeps their face
and build instead of being re-invented from prose every shot.
...
THE ONE RULE OF ref2va HERE: a shot carries EITHER references or the last-frame
handoff, never both. They are two different task conditionings competing for the
same `cond_video_latents` slot in comfy/model_base.py, and the refs branch
overwrites the keyframe branch while the packed layout still reserves rows for
both. `ref_mode` decides which shots take which:

  first shot                -- refs establish the cast in shot 1, every later
                               shot uses the handoff. Continuity unbroken.
  ...
  every shot + handoff ref  -- refs every shot, plus the previous shot's last
                               frame as one more reference. Continuity comes
                               back as a soft signal, still one ref2va task.

The tokenizer labels the references `<Picture 1>`..`<Picture 4>` in input order
and appends your prompt after them, so a prompt can bind one to a name
("Kristy, <Picture 1>, walks in") -- or say nothing and let them act as a
general appearance anchor.
...
NODE_CLASS_MAPPINGS = {
    "H3LongVideosREF2VA": H3LongVideosREF2VA,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "H3LongVideosREF2VA": "H3 Long Videos REF2VA",
}
